...
class DashboardController extends Controller
{
    public function stats(Request $request)
    {
        ...
        // Monthly compliance for chart (last 5 months)
        $complianceChart = collect(range(4, 0))->map(function ($monthsAgo) use ($pt) {
            $date  = Carbon::now()->subMonths($monthsAgo);
            $total = ExerciseSession::whereHas('client', fn($q) =>
            ...
            return [
                'month'      => $date->format('M'),
                'compliance' => min($total * 5, 100), // normalised rough estimate
            ];
        });

        // Earnings
        $commissionRate = 0.15;
        $activeSubs = Subscription::whereHas('client', fn($q) =>
        $q->where('physiotherapist_id', $pt->id)
        )
            ->where('status', 'active')
            ->get();
        ...
        $totalSessions = ExerciseSession::whereHas('client', fn($q) =>
        $q->where('physiotherapist_id', $pt->id)
        )
            ->where('status', 'completed')
            ->count();
